...
static int __init bt_kprobe_init(void)
{
...
static void __exit bt_kprobe_exit(void)
{
    unregister_kprobe(&kp);
    printk(KERN_INFO "s0s bt: Kprobe unregistered\n");
}

module_init(bt_kprobe_init);
module_exit(bt_kprobe_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Sos-Team");
MODULE_DESCRIPTION("Bluetooth monitor with MAC tracking only");
