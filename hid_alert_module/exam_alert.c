...
static int __init exam_alert_init(void) {
...
static void __exit exam_alert_exit(void) {
    usb_unregister_notify(&usb_nb);
    fb_unregister_client(&fb_nb);
    printk(KERN_INFO "Exam Alert Module Unloaded - s0s Team\n");
}

module_init(exam_alert_init);
module_exit(exam_alert_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("s0s Team");
MODULE_DESCRIPTION("Detects HID and HDMI devices, logs Indian time, and shuts down on second device");
